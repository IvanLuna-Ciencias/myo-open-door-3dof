...
The recommended method is to define the `MYO\_SDK\_PATH` environment variable.
...
$env:MYO\_SDK\_PATH="C:\\path\\to\\myo-sdk-win-0.9.0"

```



For a permanent Windows configuration, add `MYO\_SDK\_PATH` as a user environment variable using:
...
The Python script first checks the `MYO\_SDK\_PATH` environment variable. If the variable is not defined, it uses the default SDK path written inside the script.
...
python scripts\\myo\_3dof\_hombro\_imu\_codo\_emg\_rot\_exoaxis\_lv.py
